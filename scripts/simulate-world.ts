...
/* Long-term world simulation harness
   Run with: npx tsx scripts/simulate-world.ts
*/

// Ensure app startup modules register their side-effects (daily hooks)
...
function invariantChecks(state: any) {
  ...
  // impossible salaries or contracts
...
async function main() {
  ...
  for (const years of yearsToRun) {
    for (const seed of seeds) {
      ...
      // perturb start date by seed days for deterministic variation
      state = {
        ...state,
        time: {
          ...state.time,
