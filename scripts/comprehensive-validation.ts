...
addResult(
  "Only top-tier clubs in Europe initially",
...
addResult(
  "Transfers attempted",
  completedTransfers.length > 0 || transferEvents.length > 0,
  `${completedTransfers.length} completed, ${transferEvents.length} events`,
);

// Verify transferred players are in correct clubs
let transferErrors = 0;
for (const event of transferEvents) {
  const match = event.description?.match(/(\w+.*?\w+)\s+moved.*?\s->\s(.*?)\s+for/);
  if (match) {
    const playerName = match[1];
    const targetClub = match[2];
    // Find player by name (rough check)
    ...
    if (transferredPlayer) {
      ...
      if (!playerClub?.name.includes(targetClub)) {
        transferErrors++;
      }
    }
  }
...
addResult(
  "Transfer roster updates correct",
  transferErrors === 0,
  `${transferErrors} mismatches`,
  transferErrors === 0 ? "info" : "warning",
);
...
addResult(
  "Retirements possible (mid-season skips, season 2+ triggers)",
  nowRetired >= 0,
  `${nowRetired} retired after season 1`,
  "info",
);

addResult("Active player pool maintained", stillActive > 4000, `${stillActive} active players`);
...
addResult(
  "Prospect generation reasonable (<50% of roster)",
  totalProspects < clubs.length * 24 * 0.5,
  `${totalProspects} prospects (limit: ${clubs.length * 24 * 0.5})`,
);
