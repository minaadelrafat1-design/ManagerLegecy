      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "20px" }}>
        ...
        {upcoming && opponent ? (
          <div style={{ marginBottom: 32 }}>
            <ModSectionHead title="Next Match" subtitle={`Matchday ${upcoming.matchday}`} divider />
            <ModFixtureCard
              homeTeam={
                upcoming.homeClubId === currentClub.id
                  ? currentClub.shortName || currentClub.name
                  : opponent?.shortName || opponent?.name || "Away"
              }
              awayTeam={
                upcoming.homeClubId === currentClub.id
                  ? opponent?.shortName || opponent?.name || "Home"
                  : currentClub.shortName || currentClub.name
              }
              date={upcoming.date}
              time="TBD"
                ...
                .map((match) => {
                  ...
                  return (
                    <ModFixtureCard
                      ...
                      {...(match.scoreHome !== undefined ? { homeScore: match.scoreHome } : {})}
                      {...(match.scoreAway !== undefined ? { awayScore: match.scoreAway } : {})}
                      date={match.date}
                      {...(league?.name !== undefined ? { competition: league.name } : {})}
                      isManager={isHome}
                    />
                  );
                })}
